...
interface HeroSectionProps {
  ...
  categories: any[];
  handleCategoriesSelected: (selectedCategories: string[]) => void;
...
interface Category {
  ...
  description?: string;
  title: string;
  childCategories?: { slug: { current: string } }[];
}

const HeroSection: React.FC<HeroSectionProps> = ({ selectedCategories, categories, handleCategoriesSelected, handleSearchQuery, query }) => {
  const [visibleCategory, setVisibleCategory] = useState<string | false>(false);
  const [dropdownPosition, setDropdownPosition] = useState({ top: 0, left: 0 });
  const [selectedCategoryChildren, setSelectedCategoryChildren] = useState<any[]>([]);
  const [fadeIn, setFadeIn] = useState(false);
  const [filteredSelectedCategories, setFilteredSelectedCategories] = useState<string[]>([]);
  ...
  useEffect(() => {
    if ((cat as Category)?.childCategories) {
      const filteredCategories = selectedCategories.filter(selectedCategory =>
        (cat as Category).childCategories!.some(childCategory => childCategory.slug.current === selectedCategory)
      );
  ...
  return (
    <section className=" bg-cover bg-center relative" style={{ backgroundColor: '#FFFFFF', height: '122px' }}>
      <div className="flex flex-col justify-center items-center text-white" >
        <div className="flex overflow-x-auto my-4 w-full items-center lg:justify-center">
          ...
          <div className={`flex items-center cursor-pointer transition-width duration-500`}>
            <input
              type="text"
              className={`ml-2 p-2 border border-gray-300 rounded-full text-black h-[48px] transition-all duration-500 ease-in-out ${searchActive ? 'w-[200px]' : 'w-0'
                }`}
              value={query}
              onChange={handleSearchChange}
              placeholder="Search..."
              style={{ opacity: searchActive ? 1 : 0, paddingLeft: searchActive ? '12px' : '0px' }}
            />
            <span
              className={`search-icon flex items-center justify-center transition-all duration-500 ease-in-out ${searchActive ? '-ml-12 text-black' : 'text-gray-600'
                }`}
